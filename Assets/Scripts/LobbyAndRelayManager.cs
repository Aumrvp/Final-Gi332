...
public class LobbyAndRelayManager : MonoBehaviour
{
    ...
    [Tooltip("ชื่อ Scene ของหน้า Lobby (เมื่อจบด่านสุดท้ายจะกลับมาที่นี่)")]
    ...
    private void Awake()
    {
        ...
        {
            // โหลด MainMenu ใหม่หลังจบเกม → รับช่วงต่อจาก Manager เดิม (เก็บ state, ทิ้งตัวเก่า)
            // ที่ต้องทำแบบนี้เพราะปุ่มใน scene ผูก OnClick กับ component ตัวใหม่
            // ถ้าเรา Destroy ตัวใหม่ปุ่มจะไม่ทำงาน
            currentLevelIndex = Instance.currentLevelIndex;
            currentLobby = Instance.currentLobby;
            returningToLobby = Instance.returningToLobby;
            if (Instance.heartbeatSource != null)
            {
                Instance.heartbeatSource.Cancel();
                Instance.heartbeatSource.Dispose();
                Instance.heartbeatSource = null;
            }
    ...
    async void Start()
    {
        bool wasReturning = returningToLobby;
        ...
        if (wasReturning)
        {
            ...
            if (hostStartGameButton != null && NetworkManager.Singleton != null)
                hostStartGameButton.SetActive(NetworkManager.Singleton.IsServer);
        }
        else
        {
        ...
        // สำหรับ host: รีสตาร์ท heartbeat ถ้ายังอยู่ในห้อง (ตัวเก่าโดน cancel ไปแล้วใน Awake)
    ...
    {
        return levelScenes.Count == 0 || currentLevelIndex >= levelScenes.Count - 1;
    }
